...
extern "C"
{
	...
	* @return ERR_SUCC 成功 其他失败
	*/
	 errno_t get_user_frame_data(const JKHD *handle, int id, CartesianPose *tcp);
	...
	* @return ERR_SUCC 成功 其他失败
	*/
	 errno_t set_admit_ctrl_config(const JKHD *handle, const int axis, const int opt, const double ftUser, const double ftConstant, const int ftNnormalTrack, const double ftReboundFK);
	...
	* @return ERR_SUCC 成功 其他失败
	*/
	 errno_t set_block_wait_timeout(const JKHD* handle, float seconds);
	...
	* @return ERR_SUCC 成功 其他失败
	*/
	 errno_t init_ftp_client_with_ssl(const JKHD* handle, char* password);
